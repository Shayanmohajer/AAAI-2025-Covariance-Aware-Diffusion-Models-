...
class GaussianDiffusion:
    ...
    def p_mean_variance(self, model, x, t):
        ...
        if t > 0:
            model_output_p = model(x, self._scale_timesteps(t-1))
            model_output_p, _ = torch.split(model_output_p, x.shape[1], dim=1)
        else:
            model_output_p = 0
        ...
        sec_der =  model_output - model_output_p
...
class DDPM(SpacedDiffusion):
    def p_sample(self, model, x, t):
        ...
        return {'sample': sample, 'pred_xstart': out['pred_xstart'], 'sec_der': out['sec_der']}
